...
linprog::linprog(const mat &A, const vec &b, const vec &c)
{
    prob = problemData(A, b, c);
}

void linprog::initialize()
{
    iter = 0;
    totalTime = 0.0;
    residual = 0.0;
...
void linprog::initialPoint()
{
    ...
    //x = prob.A.transpose() * coeffM.jacobiSvd(ComputeThinU | ComputeThinV).solve(b);

    // min norm(s) s.t. A'*y + s - prob.Qx = c
    y = coeffM.llt().solve(prob.A*prob.c);
    //y = coeffM.jacobiSvd(ComputeThinU | ComputeThinV).solve(A*c);
...
void linprog::calSearchDriection()
{
    /*
...
void linprog::_getDirections(vec& Rm, Factorization& factor)
{
...
void linprog::printOPT()
{
    cout<<"----------------------------"<<endl;
    cout<<"Optx:"<<endl;
    for (int i = 0; i < prob.n; i++)
        cout<<setw(5)<<i + 1<<setw(10)<<x(i)<<endl;
}

void linprog::startTimer()
{
    timer.start();
}

void linprog::endTimer()
{
    timer.stop();
